...
class TinyTag(object):
    ...
    def has_all_tags(self):
        ...
        return all((self.track, self.track_total, self.title,
                    ...
                    self.year, self.genre, self.trim, self.gain))
    ...
    def _set_field(self, fieldname, bytestring, transfunc=None, overwrite=True):
        ...
        if DEBUG:
            stderr('Field "%s" is "%s"' % (fieldname, value))
    ...
    def _set_trim(self, bytestring, transfunc=None):
        value = bytestring if transfunc is None else transfunc(bytestring)
        sString = str(value)
        devide = sString.split('\x00', 1)
        if devide[0] == 'trim' or devide[0] == 'Trim':
            setattr(self, 'trim', devide[1].replace('x',''))
        if devide[0] == 'trim2' or devide[0] == 'Trim2':
            setattr(self, 'trim2', devide[1].replace('x',''))
        if devide[0] == 'gain' or devide[0] == 'Gain':
            setattr(self, 'gain', devide[1].replace('x',''))
    ...
    def update(self, other):
        ...
        for key in ['track', 'track_total', 'title', 'artist',
                    ...
                    'genre', 'disc', 'disc_total', 'trim', 'trim2', 'comment', 'gain']:
    ...
    def _unpad(self, s):
        ...
        if '\x00' in s:
            ...
            i = -1
            while looking:
                if i + 1 >= len(s) or '\x00' not in s[:i + 1]:
                    looking = False
                else:
                    print(i)
                    print(s)
...
class ID3(TinyTag):
    ...
    def _parse_frame(self, fh, id3version=False):
        ...
        if frame_size > 0:
            ...
            if fieldname == 'trim':
                self._set_trim(content, self._decode_string)
            if fieldname and fieldname != 'trim':
